    ...
    temp_msg = [
        {
            ...
            мне нужен ответ в формате целого числа без иных символов,
            ответ содержит текущую или последнюю известную температуру по прогнозу погоды,
            в градусах цельсия,
            если прогноз погоды не доступен по любым причинам,
            то считаем, что температура в ответе равна 0""",
        },
        {
            "role": "user",
            "text": name,
    ...
    train_msg = [
        {
            ...
            мне нужен ответ в формате целого числа без иных символов,
            ответ содержит количество килокалорий,
            которое сжигает указанная тренировка за одну минуту,
            если не удается найти информацию о сжигании калорий,
            то считаем, что в ответе она равна 0""",
        },
        {
            "role": "user",
            "text": name,
...
async def get_gpt_advice(calories: int) -> str:
    train_msg = [
        {
            "role": "system",
            "text": """Я буду писать количество килокалорий,
            которые мне необходимо потребить сегодня,
            мне нужно составить сбалансированный рацион питания,
            который примерно равен к-ву указанных калорий,
            ответ содержит только список продуктов питания или блюд,
            калорийность, а так же краткий совет,
